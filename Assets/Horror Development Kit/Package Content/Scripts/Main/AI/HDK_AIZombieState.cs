...
// ----------------------------------------------------------------------------------------
public abstract class HDK_AIZombieState : HDK_AIState 
{
	// Private
	protected int 						_playerLayerMask 	= -1;
	protected int 						_bodyPartLayer		= -1;
	protected int						_visualLayerMask	= -1;
	protected HDK_AIZombieStateMachine _zombieStateMachine = null;
	...
	// -------------------------------------------------------------------------------------
	public override void SetStateMachine(HDK_AIStateMachine stateMachine )
	{
		if (stateMachine.GetType () == typeof(HDK_AIZombieStateMachine)) 
		{
			
			base.SetStateMachine (stateMachine);
			_zombieStateMachine = (HDK_AIZombieStateMachine)stateMachine;
	...
	public override void OnTriggerEvent( AITriggerEventType eventType, Collider other )
	{
		...
		if (eventType != AITriggerEventType.Exit) 
		{
			...
			if (other.CompareTag ("AI Flashlight") && curType != AITargetType.Visual_Player) 
			{
			...
			if (other.CompareTag ("AI Sound Emitter")) 
			{
				...
				Vector3 soundPos;
				float   soundRadius;
                HDK_AIState.ConvertSphereColliderToWorldSpace( soundTrigger, out soundPos, out soundRadius ); 
	...
	protected virtual bool	ColliderIsVisible( Collider other,  out RaycastHit hitInfo, int layerMask=-1 )
	{
        ...
        for (int i = 0; i < hits.Length; i++)
        {
            ...
            if (hit.distance < closestColliderDistance && GameObject.Find("Player"))
            {
