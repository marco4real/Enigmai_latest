...
// ----------------------------------------------------------------------
public abstract class HDK_AIStateMachine : MonoBehaviour 
{
	// Public
	...
	// Protected
	protected HDK_AIState _currentState						=	null;
	protected Dictionary< AIStateType, HDK_AIState> _states			=	new Dictionary< AIStateType, HDK_AIState>();
	...
	[SerializeField] 	protected HDK_AIWaypointNetwork _waypointNetwork 	= 	null;
	...
	protected virtual void Start()
	{
		...
		{
            HDK_AISensor script = _sensorTrigger.GetComponent<HDK_AISensor>();
			if (script!=null)
			{
        ...
        HDK_AIState[] states = GetComponents<HDK_AIState>();

		// Loop through all states and add them to the state dictionary
		foreach(HDK_AIState state in states )
		{
		...
		if (_animator)
		{
            HDK_AIStateMachineLink[]  scripts	=	_animator.GetBehaviours<HDK_AIStateMachineLink>();
			foreach(HDK_AIStateMachineLink script in scripts )
			{
	...
	public virtual void TakeDamage( Vector3 position, Vector3 force, int damage, Rigidbody bodyPart, int hitDirection=0 )
	{
